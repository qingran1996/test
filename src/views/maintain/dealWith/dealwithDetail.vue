<template>
	<div class="dealwithDetail">
		<van-form ref="detailform">
			...
			<van-field v-model="detailForm.problemConfirmerName" name="problemConfirmerName" label="确认人" placeholder="确认人" disabled></van-field>
			...
			<van-field v-if="workshow" v-model="detailForm.weixiuPerson" name="weixiuPerson" label="维修人" placeholder="维修人" disabled border></van-field>
			<van-field v-if="workshow" v-model="detailForm.wxcontent" class="problem" name="wxcontent" label="维修意见" placeholder="" type="textarea" autosize
			 disabled></van-field>
			<van-field v-if="workshow" v-model="detailForm.isDanger" name="isDanger" label="是否危险作业" placeholder="是否危险作业" disabled border></van-field>
		</van-form>
		<!-- 维修工单 -->
		<van-form v-if="workshow" ref="workOrder">
			...
			<van-field v-model="workOrder.content" class="problem" name="content" label="维修内容" placeholder="维修内容" type="textarea" autosize
			 disabled></van-field>
			...
			<van-field v-show="checkbox" class="problem" v-model="workOrder.problem" name="problem" label="遗留问题内容" placeholder="遗留问题内容" type="textarea"
			 autosize disabled></van-field>
	...
	export default {
		...
		mounted() {
			...
			this.workOrder = {
				...
				weixiuResultPerson: data.confirmResultPersonName!=undefined?data.confirmResultPersonName:'无需确认',
				weixiuResultType: ''
			}
			if (data.workNoteStatus==3) {
				this.workshow = false
			} else {
				this.workshow = true
			}
