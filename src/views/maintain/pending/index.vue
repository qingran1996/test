		...
		methods: {
			gotodetail(item) {
				...
				this.getEMDetail(json).then(res => {
						if (res.data.code === 0) {
							...
							console.log(data)
							if (data.stageId == 2) { //流程2，自己传自己进入操作界面，自己传别人进入详情界面
								if (data.currentAssigent != localStorage.getItem('userId')) {
									this.$router.push({
										path: '/lookDetail',
										query: {
							...
							} else if (data.stageId == 3) { //流程3，自己传自己进入操作界面，自己传别人进入详情界面
								if (data.currentAssigent != localStorage.getItem('userId')) {
									this.$router.push({
										path: '/lookDetailTwo',
										query: {
							...
							} else { //流程4，自己传自己进入操作界面，自己传别人进入详情界面
								if (data.currentAssigent != localStorage.getItem('userId')) {
									this.$router.push({
			...
			getData() {
				...
				this.$axios.post('/w/equipmentMaintenanceBussiness/getWorkNoteList', this.jsonData).then(res => {
						if (res.data.code === 0) {
							console.log(res.data.data.workNoteListResponses)
							let that = this
