<template>
	<div class="tutelagecheck">
		<van-form ref="addform">
			...
			<!-- <van-field class="fieldbottom fieldmargin" center v-model="addForm.checksign" name="checksign" label="签名确认" placeholder=""
			 disabled>
				<template #right-icon>
			...
			<van-field v-model="addForm.safemanremarks" @input="showremarks" disabled name="losscontent" label="安全员审核意见" placeholder="请输入安全员审核意见"
			...
			<van-field class="fieldbottom" center required v-model="addForm.a" name="a" label="监护人签名" placeholder="" disabled>
				<template #label>
					<div class="nameshow" style="margin-left: 10px;">监护人签名<span style="color: #008000;">({{addForm.tutelageman}})</span></div>
					<div class="address" style="color: #c8c9cc;margin-left: 10px;">{{guardershow.address}}</div>
				</template>
				<template #right-icon>
					<van-icon v-show="guardershow.writeType==0" @click="signcs" class="addmore" name="add" color="#1989fa" size="30" />
					<van-image v-show="guardershow.writeType==1" width="40" height="40" fit="cover" :src="guardershow.filepath" />
					<!-- <span v-show="safemanshow.writeType==1">已签名</span> -->
				</template>
			</van-field>

			<van-field class="fieldbottom" center required v-model="addForm.b" name="b" label="施工方签名" placeholder="" disabled>
				<template #label>
					<div class="nameshow" style="margin-left: 10px;">施工方签名<span style="color: #008000;">({{addForm.buildName}})</span></div>
					<div class="address" style="color: #c8c9cc;margin-left: 10px;">{{constructedManagershow.address}}</div>
				</template>
				<template #right-icon>
					<van-icon v-show="constructedManagershow.writeType==0" @click="constructedManagershowcs" class="addmore" name="add"
					 color="#1989fa" size="30" />
					<van-image v-show="constructedManagershow.writeType==1" width="40" height="40" fit="cover" :src="constructedManagershow.filepath" />
					<!-- <span v-show="safemanshow.writeType==1">已签名</span> -->
				</template>
			</van-field>


			<van-field class="fieldbottom" center required v-model="addForm.c" name="c" label="安全员签名" placeholder="" disabled>
				<template #label>
					<div class="nameshow" style="margin-left: 10px;">安全员签名<span style="color: #008000;">({{addForm.safemanname}})</span></div>
					<div class="address" style="color: #c8c9cc;margin-left: 10px;">{{safetyOfficershow.address}}</div>
				</template>
				<template #right-icon>
					<van-icon v-show="safetyOfficershow.writeType==0" @click="safetyOfficershowcs" class="addmore" name="add" color="#1989fa"
					 size="30" />
					<van-image v-show="safetyOfficershow.writeType==1" width="40" height="40" fit="cover" :src="safetyOfficershow.filepath" />
					<!-- <span v-show="safemanshow.writeType==1">已签名</span> -->
				</template>
			</van-field>

			<van-field class="fieldbottom" center required v-model="addForm.c" name="c" label="项目负责人签名" placeholder="" disabled>
				<template #label>
					<div class="nameshow" style="margin-left: 10px;">项目负责人签名<span style="color: #008000;">({{addForm.applyer}})</span></div>
					<div class="address" style="color: #c8c9cc;margin-left: 10px;">{{projectershow.address}}</div>
				</template>
				<template #right-icon>
					<van-icon v-show="projectershow.writeType==0" @click="projectershowcs" class="addmore" name="add" color="#1989fa"
					 size="30" />
					<van-image v-show="projectershow.writeType==1" width="40" height="40" fit="cover" :src="projectershow.filepath" />
					<!-- <span v-show="safemanshow.writeType==1">已签名</span> -->
				</template>
			</van-field>
	...
	export default {
		...
		methods: {
			...
			saferoute() {
				...
				this.time = setTimeout(() => {
					console.log(123)
					if (this.guardershow.fileId == '' || this.constructedManagershow.fileId == '' || this.safetyOfficershow.fileId ==
						'' || this.projectershow.fileId == '') {
						this.$toast('请完成各个人员的签名')
					} else {
						let json = {
							processInstanceId: this.leader.processInstanceId,
							taskId: this.leader.taskId,
							bussinessId: this.leader.bussinessId,
							guarderBeforeSid: this.guardershow.fileId,
							constructionBeforeSid: this.constructedManagershow.fileId,
							saferBeforeSid: this.safetyOfficershow.fileId,
							applyerBofreSid: this.projectershow.fileId
						}
						console.log(json)
						this.$axios.post('/w/jobWork/monitorConfirmBeforeJW', json).then(res => {
								if (res.data.code === 0) {
									// console.log(res.data.data)
									this.$router.go(-1)
