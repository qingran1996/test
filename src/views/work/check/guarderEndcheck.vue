				...
				this.time = setTimeout(() => {
					if (this.guardershow.fileId == '' || this.constructedManagershow.fileId == '' || this.safetyOfficershow.fileId ==
						...
						let json = {
							...
							guarderAfterSpid: this.guardershow.fileId,
							constructionAfterSpid: this.constructedManagershow.fileId,
							saferAfterSpid: this.safetyOfficershow.fileId,
							applyerAfterSpid: this.projectershow.fileId
						}
						console.log(json)
						this.$axios.post('/w/jobWork/monitorConfirmAfterJW', json).then(res => {
								if (res.data.code === 0) {
									// console.log(res.data.data)
									this.$router.go(-1)
